...
userarn=$(aws iam get-user --user-name UdacityUser | jq -r .User.Arn)
...
./aws-iam-authenticator add user --userarn="${userarn}" --username=github-action-role --groups=system:masters --kubeconfig=/root/.kube/config --prompt=false
